...
<template>
  <div
    ...
    <div class="bg-[#211A14]/20">
      <nav
        class="flex gap-1 justify-between border border-[#3C3E4D]/20 bg-[#27272D/20] backdrop-blur-sm px-2 py-2 max-w-screen-xl mx-auto items-center">
        <template v-for="link in navLinks">
          <NuxtLinkLocale
            ...
            :class="
              cn(
                ...
                {
                  'grid place-content-center grayscale hover:grayscale-0 transition-all duration-200 hover:bg-white/5 aspect-square rounded-full p-1.5 h-full':
                    link.icon.includes(''),
                }
              )
            "
            ...
            <Icon v-else :name="link.icon" class="w-5 h-5" />
          </NuxtLinkLocale>
          ...
          <UiNavbarChangeLocale v-if="link.type === 'i18n'" />
        </template>
      </nav>
    </div>
  </div>
